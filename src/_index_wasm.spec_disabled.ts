...
test("Can create a detector", async () => {
  const factory = createDetectorFactory(all);
  const detector = await createDetector(factory);
...
test("Detects German (de) language correctly, given 55 candidates", async () => {
  const lang = await detectLanguage("Hallo, Welt, wie geht es Dir?", all);
...
test("Detects English (en) language correctly, given 55 candidates", async () => {
  const lang = await detectLanguage("Hello, World", all);
...
describe("Detector Tests", () => {
  ...
  test("Test detector1", async () => {
    const detect = await createDetector(factory);
    detect.append("a");
    expect(await detect.detect()).toBe("en");
  });

  test("Test detector2", async () => {
    const detect = await createDetector(factory);
    detect.append("b d");
    expect(await detect.detect()).toBe("fr");
  });

  test("Test detector3", async () => {
    const detect = await createDetector(factory);
    detect.append("d e");
    expect(await detect.detect()).toBe("en");
  });

  test("Test detector4", async () => {
    const detect = await createDetector(factory);
    detect.append("\u3042\u3042\u3042\u3042a");
...
test("Can detect large corpus English (en) language correctly, given 55 candidates", async () => {
  const lang = await detectLanguage(englishCorpusText, all);
  expect(lang).toEqual("en");
});
